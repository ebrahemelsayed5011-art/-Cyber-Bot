...
// ==========================================
// كود ذكي ومستقل لتشغيل أمر الإرسال الجماعي (Broadcast)
// ==========================================
import { REST, Routes, SlashCommandBuilder, PermissionFlagsBits } from 'discord.js';

function startBroadcastSystem(botClient) {
    if (!botClient) return;

    // 1. تسجيل الأمر تلقائياً فور تشغيل البوت
    botClient.once('ready', async () => {
        try {
            console.log('🔄 [System] Registering broadcast command...');
            const commandData = new SlashCommandBuilder()
                .setName('broadcast')
                .setDescription('إرسال رسالة جماعية لجميع أعضاء السيرفر في الخاص دفعة واحدة')
                .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
                .addStringOption(opt => opt.setName('رسالة').setDescription('نص الرسالة المراد إرسالها').setRequired(true))
                .toJSON();

            const rest = new REST({ version: '10' }).setToken(botClient.token);
            await rest.put(Routes.applicationCommands(botClient.user.id), { body: [commandData] });
            console.log('✅ [System] Broadcast command registered successfully!');
        } catch (e) {
            console.log('⚠️ [System] Registration info:', e.message);
        }
    });

    // 2. الاستماع لتنفيذ الأمر عند كتابته بالسيرفر
    botClient.on('interactionCreate', async (interaction) => {
        ...
        const msgText = interaction.options.getString('رسالة');
        ...
        try {
            ...
            await interaction.editReply({ content: '❌ حدث خطأ أثناء جلب قائمة الأعضاء.' });
        }
    });
}

// فحص صامت وخلفي لالتقاط متغير البوت من النظام وتشغيل الميزة فوراً دون إحداث تعارض
if (typeof setInterval !== 'undefined') {
    let checkInterval = setInterval(() => {
        let activeBot = typeof client !== 'undefined' ? client : (typeof bot !== 'undefined' ? bot : null);
        if (activeBot) {
            startBroadcastSystem(activeBot);
